      ...
      <div className="flex w-full pt-20 px-28 shadow-lg text-white bg-[#0f0f0f]">
        <div className="mr-3">
          ...
          <div className='my-3 '>
            ...
            <div className='flex justify-between mt-2'>
              ...
              <div className='flex items-center '>
                ...
                <div className='mx-1'>
                  ...
                  <p className='text-sm text-zinc-300 font-medium'>{subscribers} subscribers</p>
                </div>
                <button className='' onClick={() => setClickEvent({ ...clickEvent, subscribe: !clickEvent.subscribe })} >
                  {clickEvent.subscribe === true ? <div className='flex py-[0.38rem] px-4 rounded-full bg-zinc-800 hover:bg-zinc-700 ml-4 text-gray-200 font-medium'>
            ...
            <div className={showMore === true ? 'py-2 px-2 rounded-xl bg-zinc-800 mt-4' : 'py-2 px-2 rounded-xl bg-zinc-800 hover:bg-zinc-700 mt-4'}>
              <p className='font-medium'>{views} views <span className="text-xl font-bold " >.</span>    {time} </p>
              {showMore === true ? <div className=''>
                <p>{video?.snippet?.description}</p>
                <button className='mx-1 mt-8 font-medium 
                  ' onClick={() => setShowMore(false)}>Show less</button>
              </div> :
                <div className=''>
                  <span>{video?.snippet?.description.slice(0, 150)}</span>
                  <button className='mx-2 font-medium
                  ' onClick={() => setShowMore(true)}>...more</button>
                </div>
              }
            </div>
          </div>
